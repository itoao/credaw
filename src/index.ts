...
const profileExists = async (profileName: string, filePath: string): Promise<boolean> => {
  try {
...
const writeAWSCredentials = async (profileName: string, awsAccessKeyId: string, awsSecretAccessKey: string) => {
...
const writeAWSConfig = async (profileName: string, region: string) => {
  const configData = `[profile ${profileName}]\nregion=${region}\n`
  try {
    await fs.ensureFile(configPath)
    let existingContent = await fs.readFile(configPath, 'utf-8')
